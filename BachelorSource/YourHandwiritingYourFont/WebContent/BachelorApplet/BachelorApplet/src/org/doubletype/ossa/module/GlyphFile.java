...
//		saveGlyphFile();
	 ...
	 */
	public GlyphFile(File a_dir, String a_name) {
		super();
		init(getClass().getResource(s_emptyFileName));
		setGlyphTitle(a_name);
		m_fileName = createFileName(a_dir, a_name);
...
//		saveGlyphFile();
	...
	private XContour [] toContours() {
		...
		ArrayList<XContour> list = new ArrayList<XContour>();
	...
	public boolean hit(Rectangle2D a_rect,
	        ...
	        boolean a_single) {
		if (a_rect == null 
		    || GlyphAction.getAction() == GlyphAction.PREVIEW_ACTION) {
			return false;
		} // if
	...
	public void addInvokeArg(int a_type) {
		...
		XInvokeArg arg = new XInvokeArg();
		arg.setName("New Arg");
		arg.setContent(0.0);
				
		XInvokeVarg varg = new XInvokeVarg();
		varg.setName("New Varg");
		varg.setSrc("Existing Var");	
		
		if (a_type == InvokeTableModel.MODULE_ARG
			|| a_type == InvokeTableModel.MODULE_VARG)
		{
			if (!m_actives.hasActiveModule())
				return;
			XModule module = m_actives.getActiveModule();
			
			if (a_type == InvokeTableModel.MODULE_ARG) {
				module.getInvoke().addInvokeArg(arg);
			} else {
				module.getInvoke().addInvokeVarg(varg);
			} // if-else
			
			m_history.record("addInvokeVar");
			return;
		} // if
		
		if (a_type == InvokeTableModel.INCLUDE_ARG
			|| a_type == InvokeTableModel.INCLUDE_VARG)
		{
			if (!m_actives.hasActiveInclude())
				return;
			XInclude include = m_actives.getActiveInclude();
			
			if (a_type == InvokeTableModel.INCLUDE_ARG) {
				include.getInvoke().addInvokeArg(arg);
			} else {
				include.getInvoke().addInvokeVarg(varg);
			} // if-else
	...
	public void removeInvokeArg(int a_type, int a_index) {
			...
			|| a_type == InvokeTableModel.MODULE_VARG)
		{
			...
			if (a_type == InvokeTableModel.MODULE_ARG) {
				module.getInvoke().removeInvokeArg(a_index);
			} else {
				module.getInvoke().removeInvokeVarg(a_index);
			} // if-else
			
			m_history.record("removeInvokeArg");
			return;
		} // if
			...
			|| a_type == InvokeTableModel.INCLUDE_VARG)
		{
			...
			if (a_type == InvokeTableModel.INCLUDE_ARG) {
				include.getInvoke().removeInvokeArg(a_index);
			} else {
				include.getInvoke().removeInvokeVarg(a_index);
			} // if-else
