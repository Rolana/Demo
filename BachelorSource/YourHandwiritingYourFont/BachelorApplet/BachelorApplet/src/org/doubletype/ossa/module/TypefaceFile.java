...
public class TypefaceFile extends GlyphFile {	
	...
	private File m_dir;
	private Hashtable<String, Integer> m_nameToIndeces = new Hashtable<String, Integer>();
	private File m_ttfFile;
	private Font m_font = null;
	private File m_binFolder;
	
	public TypefaceFile(String a_name, File a_dir) {
...
//		m_ttfFile = new File(m_binFolder, fileName);
	    m_ttfFile = new File(fileName);
	}
	
	public GlyphFile createGlyph(long a_unicode) {
		String name = UCharacter.getName((int) a_unicode);
		if (name == null) {
		...
		GlyphFile a_file = new GlyphFile(
...
//		} // if
	...
	public void addBasicLatinGlyphs() {
		String basicLatin = UCharacter.UnicodeBlock.BASIC_LATIN.toString();
...
//		saveGlyphFile();
	...
	public void addGlyph(GlyphFile a_file) {		
		...
		m_glyph.getBody().addNGlyphFile(a_file);
...
// --------------------------------------------------------------------	
	 ...
	 */
	public byte[] buildTTF(boolean a_isDebug) throws Exception {		
		String randomString = UUID.randomUUID().toString().substring(0, 4);
		
		File tempFile = new File(getGlyphTitle() + "_" + randomString + k_dotTtf);
		File target;
		...
		writer = new FontFileWriter(target);
	...
	private void loadGlyphs(FontFileWriter a_writer) throws Exception {
		...
		for (GlyphFile fileName: m_glyph.getBody().getGlyphFiles()) {			
			GlyphFile glyphFile = fileName;
